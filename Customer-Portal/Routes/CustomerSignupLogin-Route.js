...
const { CustomerCardStatusSchema } = require('../Models/CustomerCardStatus.Model');
const { CustomerSubscriptionsSchema } = require('../Models/CustomerSubscriptions.Model');
const { CustomerBillingAddressSchema } = require('../Models/CustomerBillingAddress.Model');
...
const { CustomerDeliveryAddressSchema } = require('../Models/CustomerDeliveryAddress.Model');

const { MealPackagesSchema } = require('../../MasterAdmin-Portal/Models/MMealPackage.Model');

var jwt = require('jsonwebtoken');
const { validationResult } = require("express-validator");
const { jwtSignin, jwtVerifyToken, authenticateToken } = require('../../middleware/jwt');
...
router.post('/CustomerSignup', CustomerSignUpValidations, async function (req, res, next) {
  ...
  if (!errors.isEmpty()) {
    ...
    try {
      ...
      if(stripeCustomer.data.length > 0){
        let del = await stripe.customers.del(
     ...
     let CreateStripeCustomer = await   stripe.customers.create(

        {
            ...
            'msg': 'Successfully Created ','cust_id':CreateStripeCustomer.id });
             //sending email from
...
router.post('/CustomerSubscription', CustomerSubscriptionValidations, async function (req, res, next) {
    ...
    try{
      ...
      let customer = await CustomerSignupSchema.findOne({ email: req.body.email, stripe_customer_id:body.stripe_customer_id });

      if (customer == null) {
          errorExist = true;
          errors.errors.push({ "msg": 'Email and stripe customer id does not match.' });
      ...
      let delivery_address = await CustomerDeliveryAddressSchema.findOne({ email: req.body.email });
      ...
      if(attach){
        ...
        let query = {email : req.body.email},
            update = { customer_stripe_id : body.stripe_customer_id,  stripe_payment_method_id : body.stripe_payment_method_id},
      ...
      let existingSubscription = await CustomerSubscriptionsSchema
                                       .find({ stripe_customer_id: body.stripe_customer_id,
                                              status:true });
      ...
      let mealPackage = await MealPackagesSchema
                                       .find({ stripe_plan_id: body.stripe_plan_id, status:  true});
      ...
      if(body.discount_code){
        free_plates_discount = free_plates_discount_codes.find(discount=>discount.code==body.discount_code)
      }
      ...
      let admin = new CustomerSubscriptionsSchema({
       ...
       total_plates_tobe_serverd : mealPackage[0].plates + free_plates_discount.plates,
       // stripe_dicount_code : ,
       // discount_type : ,
      ...
      let query = {email : body.email},
          update = {
            stripe_customer_id: body.stripe_customer_id,
            package_type : mealPackage[0].package_type,
            price_perplate : ((stripe_subscription.plan.amount / 100)/mealPackage[0].plates),
      ...
      if(body.billing_address){
        ...
        await CustomerBillingAddressSchema.findOneAndUpdate(query, update, options);
      }
      ...
      'msg': 'Successfully Created ','stripe_subscription_id':stripe_subscription.id, });
    }  catch (error) {
        let str = `E11000 duplicate key error collection: FirstFoodApp.customersignups index`
        console.log(error);
        // This below code will capture error and store it to ERROR collection
...
router.post('/CustomerLogin', CustomerSignInValidations, async (req, res, next) => {
    ...
    // Check if this user already exisits
    let customer = await CustomerSignupSchema.findOne({ email: req.body.email });
    if (customer == null) {

        return res.status(200).json({ errors: [{ "msg": 'Customer does not exisits! Or deactivated, Please contact Administrator.' }] });
...
router.post('/RecurringBilling', RecurringBillingValidations, async (req, res, next) => {
    ...
    try{
        ...
        let activePlan = await CustomerPlanSchema
                                         .find({ email: body.email, status : true });

        if(activePlan.length == 0){
          return res.status(422).json({'msg':'No active plan'})
        }
        // console.log(activePlan);
        let activePlanOrdersCount = await CustomerOrderSchema
                                                  .countDocuments({
        ...
        if(subscription) {
          if(activePlanOrdersCount < subscription.total_plates_tobe_serverd){
            return res.status(200).json({activePlan : activePlan , 'msg':`Active plan has ${subscription.total_plates_tobe_serverd - activePlanOrdersCount} plates left.`});
          }
        }
        ...
        let query = {email : body.email, stripe_subscription_id: activePlan[0].stripe_subscription_id},
            update = {
              status: false,
             },
            options = { upsert: false, new: true, setDefaultsOnInsert: true };

        await CustomerSubscriptionsSchema.findOneAndUpdate(query, update, options);
        ...
        if(stripe_subscription.status != 'active'){
          //make plan inactive
          let query = {email : body.email, stripe_plan_id: activePlan[0].stripe_plan_id},
              update = {
                status: false,
        ...
        let new_subscription = new CustomerSubscriptionsSchema({
         ...
         free_plates : free_plates_discount.plates,
         freePlateCoupon_no : free_plates_discount.code,
         total_plates_tobe_serverd : activePlan[0].plates + free_plates_discount.plates,
         // stripe_dicount_code : ,
         // discount_type : ,
         total_charged : (stripe_subscription.plan.amount / 100)
        });
        ...
        return res.status(200).json({'subscription':new_subscription, 'msg':`Successfully Subscribed.`});
